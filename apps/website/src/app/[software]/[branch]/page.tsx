...
const Page = async (props: Props) => {
  const {software, branch} = props.params;
  ...
  return (
    <MainLayout header={ <Header branch={ branch } config={ config } pages={ [] } repository={ repository }/> }>
      <Flex flex={ 1 } gap={ "sm" } w={ "100%" }>
        <Sidebar branch={ branch } branches={ branches } config={ config } pages={ [] } repository={ repository }/>
        <Flex component={ Center } flex={ 1 }>
          <Document branch={ branch } config={ config } pages={ [] } repository={ repository }/>
        </Flex>
      </Flex>
    </MainLayout>
  );
